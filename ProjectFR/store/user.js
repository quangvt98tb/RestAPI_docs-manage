...
export const mutations = {
  ...
  LIST_USER_BLOCK: set('list_user_block'),
...
export const actions = {
  ...
  async getListBlockUsers({ state, commit }) {
    const { api } = state.api
    const res = { isSuccess: false }
    const token = this.$auth.$storage.getUniversal('token')
    this.$axios.setHeader('Authorization', "Bearer " + token)
    ...
    try {
      ...
      const data = await this.$axios.$post(`${api}/list-block-user`)
      commit('LIST_USER_BLOCK', data)
      res.isSuccess = true
    }
    catch (err) {
      console.log(err)
